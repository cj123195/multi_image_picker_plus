...
class GpsMetadata {
  ...
  GpsMetadata.fromMap(Map json)
      : gpsVersionID = _castAsString(json['GPSVersionID']),
        gpsLatitudeRef = _castAsString(json['GPSLatitudeRef']),
        gpsLatitude = _castAsDouble(json['GPSLatitude']),
        gpsLongitudeRef = _castAsString(json['GPSLongitudeRef']),
        gpsLongitude = _castAsDouble(json['GPSLongitude']),
        gpsAltitudeRef = _castAsString(json['GPSAltitudeRef']),
        gpsAltitude = _castAsDouble(json['GPSAltitude']),
        gpsTimeStamp = _castAsString(json['GPSTimeStamp']),
        gpsSatellites = _castAsString(json['GPSSatellites']),
        gpsStatus = _castAsString(json['GPSStatus']),
        gpsMeasureMode = _castAsString(json['GPSMeasureMode']),
        gpsDOP = _castAsDouble(json['GPSDOP']),
        gpsSpeedRef = _castAsString(json['GPSSpeedRef']),
        gpsSpeed = _castAsDouble(json['GPSSpeed']),
        gpsTrackRef = _castAsString(json['GPSTrackRef']),
        gpsTrack = _castAsDouble(json['GPSTrack']),
        gpsImgDirectionRef = _castAsString(json['GPSImgDirectionRef']),
        gpsImgDirection = _castAsString(json['GPSImgDirection']),
        gpsMapDatum = _castAsString(json['GPSMapDatum']),
        gpsDestLatitudeRef = _castAsString(json['GPSDestLatitudeRef']),
        gpsDestLatitude = _castAsDouble(json['GPSDestLatitude']),
        gpsDestLongitudeRef = _castAsString(json['GPSDestLongitudeRef']),
        gpsDestLongitude = _castAsDouble(json['GPSDestLongitude']),
        gpsDestBearingRef = _castAsString(json['GPSDestBearingRef']),
        gpsDestBearing = _castAsDouble(json['GPSDestBearing']),
        gpsDestDistanceRef = _castAsString(json['GPSDestDistanceRef']),
        gpsDestDistance = _castAsDouble(json['GPSDestDistance']),
        gpsProcessingMethod = _castAsString(json['GPSProcessingMethod']),
        gpsAreaInformation = _castAsString(json['GPSAreaInformation']),
        gpsDateStamp = _castAsString(json['GPSDateStamp']),
        gpsDifferential = _castAsInt(json['GPSDifferential']),
        gpsHPositioningError = _castAsDouble(json['GPSHPositioningError']),
        interoperabilityIndex = _castAsString(json['InteroperabilityIndex']);

  /// Indicates the version of GPS Info IFD. The version is given as 2.3.0.0.
  /// This tag is mandatory when GPS-related tags are present. Note that this
  /// tag is written as a different byte than {@link #TAG_EXIF_VERSION}.
  ...
  /// would be dd/1,mm/1,ss/1. When degrees and minutes are used and, for example,
  /// fractions of minutes are given up to two decimal places, the format
  ...
  /// would be ddd/1,mm/1,ss/1. When degrees and minutes are used and, for example,
  ...
  /// indicated as an absolute value in @link #TAG_GPS_ALTITUDE}
  final String? gpsAltitudeRef;

  /// Indicates the altitude based on the reference in
  /// {@link #TAG_GPS_ALTITUDE_REF}. The reference unit is meters.
  ...
  /// Indicates the GPS satellites used for measurements. This tag may be used
  ...
  /// measurements, value of the tag shall be set to {@code null}.
  ...
  /// Indicates the GPS measurement mode. Originally it was defined for GPS,
  /// but it may be used for recording a measure mode to record the position
  /// information provided from a mobile base station or wireless LAN as well
  /// as GPS.
  ...
  /// expressed as three unsigned rational values giving the degrees, minutes,
  /// and seconds, respectively. If latitude is expressed as degrees, minutes
  ...
  /// and minutes are used and, for example, fractions of minutes are given up
  /// to two decimal places, the format would be ddd/1, mmmm/100, 0/1.
  ...
  /// indicates the character code used, and this is followed by the name of
  /// the GPS area.
...
class DeviceMetadata {
  ...
  DeviceMetadata.fromMap(Map json)
      : make = _castAsString(json['Make']),
        model = _castAsString(json['Model']),
        software = _castAsString(json['Software']),
        makerNote = _castAsString(json['MakerNote']),
        cameraOwnerName = _castAsString(json['CameraOwnerName']),
        bodySerialNumber = _castAsString(json['BodySerialNumber']),
        lensSpecification = _castAsDoubleMap(json['LensSpecification']),
        lensMake = _castAsString(json['LensMake']),
        lensModel = _castAsString(json['LensModel']),
        lensSerialNumber = _castAsString(json['LensSerialNumber']);
  ...
  /// of number of the DSC, scanner, video digitizer or other equipment that
  ...
  /// those in {@link #TAG_IMAGE_DESCRIPTION}, and without the character code
  /// limitations of it.
  ...
  /// This tag records the serial number of the body of the camera that was
  /// used in photography as an ASCII string.
  final String? bodySerialNumber;

  /// This tag notes minimum focal length, maximum focal length, minimum F
  /// number in the minimum focal length, and minimum F number in the maximum
  /// focal length, which are specification information for the lens that was
  ...
  /// This tag records the lens’s model name and model number as an ASCII
  /// string.
  final String? lensModel;

  /// This tag records the serial number of the interchangeable lens that was
  /// used in photography as an ASCII string.
...
List<int?>? _castAsIntMap(List<dynamic>? list) {
  ...
  return list.map<int?>(_castAsInt).toList();
}
...
List<double?>? _castAsDoubleMap(List<dynamic>? list) {
  ...
  return list.map<double?>(_castAsDouble).toList();
}
